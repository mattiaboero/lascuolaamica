...
function saveCursor(c) {
  try { storageSet(CURSOR_KEY, JSON.stringify(c)); } catch (e) { debugWarn('saveCursor', e); }
}
...
function recordRewards() {
  try {
    ...
    window.SA.rewards.recordGame({
      ...
      areas: Array.from(new Set(questions.map((q) => q && (q.area || q._area)).filter(Boolean))),
...
async function clearLeaderboard() {
  const shouldClear = await askConfirm('Cancellare tutta la classifica?', {
    title: 'Classifica Educazione Civica',
