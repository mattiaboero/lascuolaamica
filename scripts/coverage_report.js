...
function generateCoverageReport() {
  ...
  SUBJECTS.forEach(subject => {
    ...
    report.push(`${'Subarea'.padEnd(25)} | ${CLASSES.map((c) => `c${c}`.padEnd(6)).join(' | ')} | Total\n`);
    report.push('|' + '-'.repeat(23) + '|' + ('-'.repeat(7) + '|').repeat(CLASSES.length) + '-'.repeat(8) + '|\n');
    ...
    report.push(`| ${'TOTAL'.padEnd(23)} | ${CLASSES.map(() => ' '.padEnd(6)).join(' | ')} | ${String(subjectTotal).padEnd(6)}\n`);
    report.push('\n');
  });
